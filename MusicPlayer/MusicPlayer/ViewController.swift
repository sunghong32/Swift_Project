...
class ViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    func initialiazePlayer() {
        ...
        do {
            ...
            print("플레이어 초기화 실패")
            print("\(error.code), 메세지 : \(error.localizedDescription)")
        }
        
        self.progessSlider.maximumValue = Float(self.player.duration)
        self.progessSlider.minimumValue = 0
        self.progessSlider.value = Float(self.player.currentTime)
    }
    ...
    func makeAndFireTimer() {
        self.timer = Timer.scheduledTimer(withTimeInterval: 0.01, repeats: true, block: { [unowned self] (timer : Timer) in
            if self.progessSlider.isTracking {return}
